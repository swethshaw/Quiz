...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Merged your fetch logic with the new env variable
      ...
      if (data.success) {
        login(data.data); // Updates your UserContext
  ...
  // Helper for your demo account feature
  ...
  return (
    <div className="min-h-screen w-full bg-[#F4F7FE] dark:bg-[#0B0F19] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <motion.div 
        ...
        className="w-full max-w-md bg-white dark:bg-slate-900 rounded-4x1 shadow-2xl shadow-violet-500/10 dark:shadow-none border border-slate-100 dark:border-slate-800 p-8 relative z-10"
      >
        ...
        {/* Demo Account Access & Registration Link */}
        <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 text-center space-y-4">
          <div>
            ...
            <button 
              ...
              className="text-sm font-bold text-slate-600 dark:text-slate-300 mt-1 hover:text-violet-600 dark:hover:text-violet-400 transition-colors"
            >
