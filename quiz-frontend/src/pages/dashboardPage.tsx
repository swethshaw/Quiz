...
export default function Dashboard() {
  ...
  const visibleTopics = useMemo(
    () => displayTopics.slice(0, visibleCount),
    [displayTopics, visibleCount],
  );
  ...
  const renderHeatmap = () => {
    ...
    if (heatmapMode === "month") {
      ...
      const maxScore = Math.max(...months.map((m) => m.score), 1); // prevent div by zero
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B0F19] text-slate-800 dark:text-slate-200 p-4 md:p-8 font-sans overflow-x-hidden transition-colors duration-300">
      <div className="mx-auto space-y-8">
        ...
        <motion.div
          ...
          className="bg-linear-to-r from-violet-600 via-violet-500 to-indigo-600 rounded-[2rem] p-8 md:p-10 text-white shadow-2xl shadow-violet-500/20 relative overflow-hidden group"
        >
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-violet-100 max-w-xl font-medium text-lg leading-relaxed">
                ...
                <span className="text-amber-300 font-black px-1">
                  #{userRank}
                </span>{" "}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 md:p-8 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-lg transition-all duration-300"
            >
              ...
              <div className="min-h-[220px]">{renderHeatmap()}</div>
            ...
            >
              <div className="flex justify-between items-center mb-8 border-b border-slate-100 dark:border-slate-800 pb-6">
                <h3 className="font-black text-xl text-slate-900 dark:text-white flex items-center gap-2">
                  <BookOpen className="text-violet-500" size={24} /> Module
                  Breakdown
                </h3>
                  ...
                  activeRooms.slice(0, 3).map((room) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() =>
                            navigate(`/peer-quiz?join=${room.code}`)
                          }
...
function StatCard({ icon, title, value, subtitle, color, bg }: any) {
  return (
    <div className="bg-white dark:bg-slate-900 p-5 md:p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
      <div className="flex items-center justify-between mb-4">
